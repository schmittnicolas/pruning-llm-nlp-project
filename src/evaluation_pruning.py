...
def calculate_energy_savings_computation(inference_time_saved, num_inferences):
    energy_saved = POWER_CONSUMPTION_INFERENCE * inference_time_saved * num_inferences  # in watt-seconds (Joules)
...
def compare_inference_time(original_model, pruned_model, nsamples, seed, seqlen, tokenizer):
    # Get data samples
    ...
    print(f"Average Inference Time for Original Model: {inference_time_original:.4f} seconds")
    print(f"Average Inference Time for Pruned Model: {inference_time_pruned:.4f} seconds")
...
def compare_models_for_prompt(original_model, pruned_model, prompt=PROMPT):
    ...
    original_output = original_model(prompt, max_length=100, num_return_sequences=1)[0]['generated_text']
    
    # Generate output from the pruned model
    pruned_output = pruned_model(prompt, max_length=100, num_return_sequences=1)[0]['generated_text']
...
def global_evaluation(args, original_model, pruned_model, tokenizer):

    original_model_perplexity = eval_perplexity(args, original_model, tokenizer)
    pruned_model_perplexity = eval_perplexity(args, pruned_model, tokenizer)
    ...
    compare_inference_time(original_model, pruned_model, args.nsamples, args.seed, args.seqlen, tokenizer)
